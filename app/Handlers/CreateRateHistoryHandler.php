...
class CreateRateHistoryHandler
{
    ...
    public function handle(string $hotelName)
    {
        $historyTable = (new HotelStayRatesHistory)->getTable();
        $rateTable = (new HotelStayRate)->getTable();
        $today = today()->format('Y-m-d');
        $now = now();
        $count = HotelStayRatesHistory::where('date_scraped', $today)
            ->where('date_of_stay', $today)
        ...
        if ($count < 1) {
            ...
            uuid(),hotel_name,date_of_stay,date_scraped,rate_per_night,id,'{$now}','{$now}' from {$rateTable} where hotel_name ='{$hotelName}' and date_scraped >= '{$today}'"
            );
        }
    }
}
